...
`/run`: Specify the source file (if the suffix is not specified as `.cpp`, the `.cpp` suffix will be automatically added), compile and run, no compilation information will be output when using this parameter.

`/trun`: Specify the source file (if the suffix is not specified as `.cpp`, the `.cpp` suffix will be automatically added) and run the executable file corresponding to the source file. Using this parameter will not output compilation information.

...
int check_main()
{
    ...
    if (get_sum_parameter("f") == 3)
    {
        ...
        if (find_file(add_namesuf(get_name(_check_in), ".cpp")))
        {
            std::cout << "\ndata_maker:";
            print_result(_NF);
            return 0;
        }
        if (find_file(add_namesuf(get_name(_check_out), ".cpp")))
        {
            std::cout << "\nstd:";
            print_result(_NF);
            return 0;
        }
        if (find_file(add_namesuf(get_name(_check_ans), ".cpp")))
        {
            std::cout << "\nans:";
            print_result(_NF);
            return 0;
    ...
    else
    {
        if (get_sum_parameter("if") == 1)
            add_file(_check_in, get_parameter("if", 1));
        if (get_sum_parameter("of") == 1)
            add_file(_check_out, get_parameter("of", 1));
        if (get_sum_parameter("af") == 1)
            add_file(_check_ans, get_parameter("af", 1));
        if (get_sum_parameter("if") == 1 && find_file(add_namesuf(get_name(_check_in), ".cpp")))
        {
            std::cout << "\ndata_maker:";
            print_result(_NF);
            return 0;
        }
        if (get_sum_parameter("of") == 1 && find_file(add_namesuf(get_name(_check_out), ".cpp")))
        {
            std::cout << "\nstd:";
            print_result(_NF);
            return 0;
        }
        if (get_sum_parameter("af") == 1 && find_file(add_namesuf(get_name(_check_ans), ".cpp")))
        {
            std::cout << "\nans:";
            print_result(_NF);
            return 0;
    ...
    if (get_sum_parameter("c") != -1)
    {
        if (get_sum_parameter("c") == 1)
        {
            ...
            if (find_file(add_namesuf(get_name(_check_chk), ".cpp")))
            {
                std::cout << "\nchecker:";
                print_result(_NF);
                return 0;
    ...
    // find file
    if (find_name(_check_in))
    {
        std::cout << "\ndata_maker:";
        print_result(_NF);
        return 0;
    }
    if (find_name(_check_out))
    {
        std::cout << "\nstd:";
        print_result(_NF);
        return 0;
    }
    if (find_name(_check_ans))
    {
        std::cout << "\nans:";
        print_result(_NF);
        return 0;
    }
    if (use_checker && find_name(_check_chk))
    {
        std::cout << "\nchecker:";
        print_result(_NF);
        return 0;
    ...
    // compile file
    if (print_compile(in, "data_maker", "-D JUDGING"))
    {
        std::cout << "\ndata_maker:";
        print_result(_CE);
        return 0;
    }
    if (print_compile(out, "std", "-D JUDGING"))
    {
        std::cout << "\nstd:";
        print_result(_CE);
        return 0;
    }
    if (print_compile(ans, "ans", "-D JUDGING"))
    {
        print_result(_CE);
        return 0;
    }
    if (print_compile(chk, "checker", "-D JUDGING"))
    {
        std::cout << "\nchecker:";
        print_result(_CE);
        return 0;
    ...
    if (find_dangerous_syscalls(in, "-D JUDGING"))
    {
        std::cout << "\ndata_maker:";
        print_result(_DS);
    }
    if (find_dangerous_syscalls(out, "-D JUDGING"))
    {
        std::cout << "\nstd:";
        print_result(_DS);
    }
    if (find_dangerous_syscalls(ans, "-D JUDGING"))
    {
        print_result(_DS);
    }
    // init monitor
    bool use_monitor = false;
    if (get_sum_parameter("e") != -1)
        use_monitor = true;
    // init compare
    bool use_compare = false;
    if (get_sum_parameter("p") != -1)
        use_compare = true;
    // init data dir
    ...
    for (int i = 1; i <= total_sum; ++i)
    {
        ...
        if (use_compare)
        {
            if (use_monitor)
            {
                if (run_monitor(in, " > \"" + appdata_path + sPATH_SE + "data" + sPATH_SE + "data.in\" " + std::to_string(i)))
                {
                    std::cout << "\ndata_maker:";
                    print_result(_TLE_O, get_time_limit() * 2);
                    continue;
                }
                if (Judge::exit_code != 0)
                {
                    std::cout << "\ndata_maker:";
                    print_result(_RE, Judge::exit_code);
                    continue;
                }
            }
            else
        ...
        else
        {
            if (use_monitor)
            {
                ...
                if (run_monitor(out, " < \"" + appdata_path + sPATH_SE + "data" + sPATH_SE + "data.in\" > \"" + appdata_path + sPATH_SE + "data" + sPATH_SE + "data.out\""))
                {
                    std::cout << "\nstd:";
                    print_result(_TLE_O, get_time_limit() * 2);
                    continue;

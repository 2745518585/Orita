...
int judge_main()
{
    ...
    if (get_sum_parameter("f") == 1)
    {
        ...
        if (find_file(add_namesuf(get_name(_run_ans), ".cpp")))
        {
            std::cout << "\nans:";
            print_result(_NF);
            return 0;
    ...
    if (get_sum_parameter("c") != -1)
    {
        if (get_sum_parameter("c") == 1)
        {
            ...
            if (find_file(add_namesuf(get_name(_run_chk), ".cpp")))
            {
                std::cout << "\nchecker:";
                print_result(_NF);
                return 0;
    ...
    // find file
    if (find_name(_run_ans))
    {
        std::cout << "\nans:";
        print_result(_NF);
        return 0;
    }
    if (use_checker && find_name(_run_chk))
    {
        std::cout << "\nchecker:";
        print_result(_NF);
        return 0;
    ...
    for (int i = 1; i <= total_sum; ++i)
    {
        ...
        if (find_file(Judge::in_file))
        {
            print_result(_NF);
            file << i << ":\t" << get_short_result(_NF) << "\n";
            continue;
        }
        if (chk != ":0" && find_file(Judge::out_file))
        {
            print_result(_NF);
            file << i << ":\t" << get_short_result(_NF) << "\n";
            continue;

...
json make_cor_parameter()
{
    json cor_parameter = {
        {"f", {1}}};
    for (int i = 1; i <= SUM; ++i)
        cor_parameter[std::to_string(i)] = {_not_define, 2};
...
int check_value(std::string str)
{
    ...
    for (int i = 0; i < str.size(); ++i)
    {
        ...
        {
            if (!check_number(i - 1) && !check_number(i + 1))
                return 1;
        }
        else if (str[i] == '-')
        {
    ...
    for (int i = 0; i < str.size(); ++i)
    {
        if (str[i] == ':')
        {
            ...
            if (j == std::string::npos)
                return 1;
            if (check_value(str.substr(i + 1, j - i - 1)))
                return 1;
            i = j;
        }
    }
...
std::string solve_value(std::string str)
{
    ...
    for (int i = 0; i < str.size(); ++i)
    {
        if (str[i] >= '0' && str[i] <= '9')
        {
            if ((i < 1 || (str[i - 1] < '0' || str[i - 1] > '9')))
                ...
                sta1[top1] = sta1[top1] * 10 + str[i] - '0';
        }
        if (str[i] == 'x')
        {
    ...
    if (len_begin == -1)
        sprintf(buf, "%d", sta1[1]);
    else
        sprintf(buf, str.substr(len_begin, str.size() - len_begin).c_str(), sta1[1]);
    return buf;
}
...
void make_scheme(int num)
{
    ...
    for (value[num] = min_value[num]; value[num] <= max_value[num]; ++value[num])
        make_scheme(num + 1);
}
